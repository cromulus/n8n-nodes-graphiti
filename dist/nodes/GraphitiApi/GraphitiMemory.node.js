        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (operation === 'initializeSession') {
                    ...
                    const currentMessage = this.getNodeParameter('currentMessage', i);
                    const maxFacts = this.getNodeParameter('maxFacts', i);
                    const centerNodeUuid = this.getNodeParameter('centerNodeUuid', i, '');
                    const messages = [{
                    ...
                    const memoryResponse = await (0, GenericFunctions_1.getMemory)(this, params);
                    returnData.push({
                        json: {
                            ...memoryResponse,
                            sessionId,
                    ...
                    const aiResponse = this.getNodeParameter('aiResponse', i);
                    const userName = this.getNodeParameter('userName', i, 'User');
                    const aiName = this.getNodeParameter('aiName', i, 'Assistant');
                    ...
                    const recentCount = this.getNodeParameter('recentCount', i);
                    const params = {
                        group_id: sessionId,
                        last_n: recentCount,
                    };
                    const historyResponse = await (0, GenericFunctions_1.getEpisodes)(this, params);
                    returnData.push({
                        json: {
                            ...historyResponse,
                            sessionId,
                    ...
                    const entityUuid = this.getNodeParameter('entityUuid', i);
                    const entitySummary = this.getNodeParameter('entitySummary', i, '');
                    const params = {
                        uuid: entityUuid,
                        group_id: sessionId,
                        name: entityName,
